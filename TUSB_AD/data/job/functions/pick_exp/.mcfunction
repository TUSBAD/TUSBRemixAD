...
execute if entity @a[distance=..50,tag=ExpTarget,limit=1] run function job:pick_exp/count
### 経験値を得られなかったプレイヤーがいた場合、経験値取得失敗メッセージを出す
execute if entity @a[tag=ExpTarget,limit=1] run function job:pick_exp/get
