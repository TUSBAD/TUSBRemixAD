...
## 1 -> 1
execute if score @s ModeState matches 1.. run function job:egg/mode/switch/a
## ..0 -> 3
execute unless score @s ModeState matches 1.. run function job:egg/mode/switch/b
scoreboard players remove @s ModeState 2

scoreboard players operation @s ShowSkill = @s CurrentMode
function job:set_display/show/
