...
# アイテムを渡しましょう
execute if score @s TUSB matches 55.. if score @s TUSB_sub matches 6.. at @s run summon minecraft:bat ~ ~0.1 ~ {Health:0.001f,active_effects:[{Id:14,Amplifier:0b,Duration:100,ShowParticles:false},{Id:7,Amplifier:0b,Duration:100,ShowParticles:false}],Passengers:[{id:"minecraft:item",Item:{id:"minecraft:chest",Count:1b,tag:{BlockEntityTag:{LootTable: "usb:chests/enjoy"},display: {Lore: ['{"text":"§eなにか§cアイテム§eが入っている"}', '{"text":"§e開ける人の§a§l運§eによって"}', '{"text":"§a§l質§eが変わる不思議なチェスト"}'], Name: '{"text":"§dお楽しみセット"}'}}}}]}
