...
# 耐性エフェクト
    execute if data storage score_damage: Argument{BypassResistance:0b} store result score $Resistance ScoreDamageCore run data get entity @s active_effects[{Id:11}].amplifier
    execute if data storage score_damage: Argument{BypassResistance:0b} if data entity @s active_effects[{Id:11}] run scoreboard players add $Resistance ScoreDamageCore 1
